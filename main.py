...
total_dinero = None #Declaramos la variable para calcular el monto total
...
def registrar_entradas(mensaje):
    ...
    """
    try:
        archivo = open("registro_entradas.txt","a", encoding="UTF-8")
...
def autos_por_dia():
    ...
    """
    try:
        archivo = open("registro_entradas.txt", "r")
        caracteres = archivo.read()
        renglones = caracteres.split()
        patron = '([A-Z]{3}[0-9]{3})|([A-Z]{2}[0-9]{3}[A-Z]{2})'
        patentes = []
        for renglon in renglones:
            buscar = re.findall(patron, renglon, re.IGNORECASE)
            for i in buscar:
                patentes.append(i[0])
                patentes.append(i[1]) 
        #strip() elimina los strings vacios de la lista "patentes"
        return [elemento.strip() for elemento in patentes if elemento.strip()]
    except IOError:
        print("Error")
...
def cantidad_autos():
    ...
    for duplicadas in patentes_duplicadas:
        veces_ingresadas = patentes_dia.count(duplicadas)
        #-----------------REVISAR-----------------
        print("---CONTADOR DE PATENTES DUPLICADAS---")
        print(f"la patente: {duplicadas}, ingresó: {veces_ingresadas} veces")
        #-----------------REVISAR-----------------
...
def registrar_en_archivo(mensaje):
    ...
    """
    try:
        archivo = open("registro_estacionamiento.txt","a", encoding="UTF-8")
        archivo.write(mensaje + "\n")
    except IOError:
        print("Error al escribir en el archivo de registro.")
